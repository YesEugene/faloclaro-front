...
export async function GET(request: NextRequest) {
  try {
    const { data: levels, error } = await getSupabaseAdmin()
      .from('levels')
      .select('*')
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: existingLevel, error: checkError } = await getSupabaseAdmin()
      .from('levels')
      .select('id')
    ...
    // Create new level
    const { data: level, error: insertError } = await getSupabaseAdmin()
      .from('levels')
      .insert({
        level_number,
        name_ru,
