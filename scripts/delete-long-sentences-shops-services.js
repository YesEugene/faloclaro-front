/**
 * Delete all long_sentence phrases for "Shops and Services" cluster
 * 
 * Usage:
 * node scripts/delete-long-sentences-shops-services.js
 */
...
async function deleteLongSentences() {
  try {
    ...
    // Get all long_sentence phrases for this cluster
    ...
    console.log(`📋 Found ${phrases?.length || 0} long sentences to delete`);

    if (!phrases || phrases.length === 0) {
      console.log('✅ No long sentences to delete');
      return;
    }
    ...
    console.log(`✅ Successfully deleted ${phrases.length} long sentences and their translations`);
    console.log(`✅ Deleted ${audioFiles.length} audio files from storage`);
