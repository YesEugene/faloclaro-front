/**
 * Delete all word phrases for "Shops and Services" cluster
 * 
 * Usage:
 * node scripts/delete-words-shops-services.js
 */
...
async function deleteWords() {
  try {
    ...
    // Get all word phrases for this cluster
    ...
    console.log(`📋 Found ${phrases?.length || 0} words to delete`);

    if (!phrases || phrases.length === 0) {
      console.log('✅ No words to delete');
      return;
    }
    ...
    console.log(`✅ Successfully deleted ${phrases.length} words and their translations`);
    console.log(`✅ Deleted ${audioFiles.length} audio files from storage`);
