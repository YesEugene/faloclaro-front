/**
 * Generate audio for Day 3 lesson vocabulary cards and phrases
 */
...
async function main() {
  console.log('🚀 Generating audio for Day 3 lesson...\n');

  // Read main YAML file
  const yamlPath = path.join(__dirname, '../Subsription/3 Day/day_03.yaml');
    ...
    for (const blockKey in blocks) {
      ...
      if (block.task_2?.options && Array.isArray(block.task_2.options)) {
        for (const option of block.task_2.options) {
          if (option.text && typeof option.text === 'string' && option.correct) {
            console.log(`   Adding task_2 option: ${option.text}`);
  ...
  for (const text of textsToGenerate) {
    ...
    // Generate audio
    const filename = `lesson-3-${sanitizeFilename(text)}.mp3`;
  ...
  console.log(`   Run: node scripts/upload-audio-lesson-day3.js`);
}
