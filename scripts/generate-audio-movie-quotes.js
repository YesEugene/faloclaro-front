/**
 * Generate audio for all cult phrases in "Cult Phrases" cluster
 * that don't have audio_url yet
 * 
 * Usage:
 * node scripts/generate-audio-movie-quotes.js
 */
...
async function main() {
  console.log('🚀 Starting audio generation for "Cult Phrases" cluster...\n');

  const cluster = await getCluster();
  console.log(`✅ Found cluster: ${cluster.name} (ID: ${cluster.id})\n`);

  // Get all cult phrases without audio
  ...
  for (let i = 0; i < phrases.length; i++) {
    ...
    const filename = `phrase-${phrase.id}-${phrase.portuguese_text.toLowerCase().replace(/[^a-z0-9]+/g, '-').substring(0, 50)}.mp3`;
