/**
 * Import short sentences from CSV file for "Making sense" cluster
 * File: Короткие предложения.csv
 * 
 * Usage:
 * node scripts/import-understanding-short-sentences.js
 */
...
const CATEGORIES_DIR = path.join(__dirname, '../Categories/4. Понимание и уточнение');
...
async function getCluster() {
  const { data: cluster, error } = await supabase
    .from('clusters')
    .select('id, name')
    .eq('name', 'Making sense')
    .single();

  if (error || !cluster) {
    console.error('❌ Cluster "Making sense" not found:', error);
    process.exit(1);
  }
...
// Main function
async function main() {
  console.log('🚀 Starting import for "Making sense" cluster (short sentences)...\n');

  // Get cluster
  console.log('🔍 Finding "Making sense" cluster...');
