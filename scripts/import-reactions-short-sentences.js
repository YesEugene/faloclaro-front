/**
 * Import short sentences from CSV file for "My take" cluster
 * File: Короткие предложения.csv
 * 
 * Usage:
 * node scripts/import-reactions-short-sentences.js
 */
...
const CATEGORIES_DIR = path.join(__dirname, '../Categories/2. Реакции и ответы');
...
async function getCluster() {
  ...
  if (error || !cluster) {
    console.error('❌ Cluster "My take" not found:', error);
    process.exit(1);
  }
...
// Main function
async function main() {
  console.log('🚀 Starting import for "My take" cluster (short sentences)...\n');

  // Get cluster
  console.log('🔍 Finding "My take" cluster...');
