/**
 * Import short sentences from CSV file for "Time and Path" cluster
 * File: Короткие предложения.csv
 * 
 * Usage:
 * node scripts/import-time-movement-short-sentences.js
 */
...
const CATEGORIES_DIR = path.join(__dirname, '../Categories/5. Время, движение, паузы');
...
async function getCluster() {
  const { data: cluster, error } = await supabase
    .from('clusters')
    .select('id, name')
    .eq('name', 'Time and Path')
    .single();

  if (error || !cluster) {
    console.error('❌ Cluster "Time and Path" not found:', error);
    process.exit(1);
  }
...
// Main function
async function main() {
  console.log('🚀 Starting import for "Time and Path" cluster (short sentences)...\n');

  // Get cluster
  console.log('🔍 Finding "Time and Path" cluster...');
