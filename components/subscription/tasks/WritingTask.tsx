    ...
    <div className="space-y-6 w-full" style={{ paddingBottom: '120px' }}>
      ...
      <div className="rounded-lg border-2 border-gray-200 p-6 w-full" style={{ backgroundColor: '#F4F5F8' }}>
        <div className="space-y-4">
          ...
          {/* Example - Show by text link */}
          {example.show_by_button && example.content && (
            <div className="space-y-2">
              {!showExample && (
                <button
                  onClick={() => setShowExample(true)}
                  className="flex items-center gap-2 text-black font-medium transition-colors hover:opacity-70"
                  style={{
                ...
                >
                  ...
                  <span>{example.button_text || (appLanguage === 'ru' ? 'Показать пример' : appLanguage === 'en' ? 'Show example' : 'Mostrar exemplo')}</span>
                </button>
              )}
