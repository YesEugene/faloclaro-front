...
export default function AttentionTask({ task, language, onComplete, isCompleted }: AttentionTaskProps) {
  ...
  return (
    <div className="space-y-6 w-full">
      ...
      {/* Completion Section - Progress with stars (same as ListeningTask) */}
      {isCompleted && (
          ...
          <div className="text-center">
            ...
            >
              {(() => {
                const taskId = task?.task_id || 4;
                if (appLanguage === 'ru') {
                  return taskId === 1 ? 'Начало' : taskId === 2 ? 'Разогрев' : taskId === 3 ? 'Понимание' : taskId === 4 ? 'Уверенность' : 'Закреплено';
                } else if (appLanguage === 'en') {
                  return taskId === 1 ? 'Start' : taskId === 2 ? 'Warm-up' : taskId === 3 ? 'Understanding' : taskId === 4 ? 'Confidence' : 'Consolidated';
                } else {
                  return taskId === 1 ? 'Início' : taskId === 2 ? 'Aquecimento' : taskId === 3 ? 'Compreensão' : taskId === 4 ? 'Confiança' : 'Consolidado';
                }
              })()}
            </p>
          </div>
